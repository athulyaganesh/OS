...
/**************************************************************/
void ignore_signal(int signum) //NO CODE TO BE RETURNED. 
{}
...
void execute(char **argv, char *raw_line_input)
{
     ...
     if (pid == 0) 
       ...
       { /* for the parent:      */
		 ...
		 int stat; 
		 waitpid(pid, &stat, WUNTRACED); //we use WAITPID() to make parent wait for the child to exit
		 // and also, wait until child is stopped with SIGSTOP 
		 
		 
		 if(WIFEXITED(stat) || WIFSIGNALED(stat)) // normal termination (child) 
		 {
       ...
       {  // Maybe once per processing loop you could try to check if there's
          ...
          if ((*line != '\0') && (*line > 31)) // If something was actually typed, then do something...
            { 
              ...
              else if (strcmp(line_argv[0], "cd") == 0)  {
                int dirCheck = 0; //initialize int value to be zero 
                if (line_argv[1] == NULL){  // if the argv[1] value is NULL, then user is asking to be in the home directory (when ONLY CD is typed). 
                  dirCheck = chdir("/home"); // chdir command: he chdir command is a system function (system call) which is used to change the current working directory
                }
                else {                      
                  dirCheck = chdir(line_argv[1]); // else the directory is whatever directory the user inputs provided it exists. If it does not, then the error code is passed on to dirCheck. 
                }
				if (dirCheck != 0){
