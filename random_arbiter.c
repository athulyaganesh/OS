...
// RANDOM RESOURCE ARBITER SOLUTION 
...
pthread_mutex_t waiter; // ATHULYAS COMMENTS: GLOBAL MUTEX WAITER WHO ACTS AS THE ARBITRATOR 
   ...
   {  
      ...
      while (MEALS_ON_TABLE > 0)
         { // In this version of the "philosopher program", the philosopher
           ...
           pthread_mutex_lock(&waiter); // ATHULYAS COMMENTS: ONLY ONE PHILOSOPHER CAN PICK AT ONCE UNTIL PHILOSOPHER PICKS BOTH UP 

           pthread_mutex_lock(&chopstick[chopstick_1]);
           pthread_mutex_lock(&chopstick[chopstick_2]);
           
           pthread_mutex_unlock(&waiter);  //unlock the waiter mutex after a philosopher has picked both up 
...
int main()
  {  int i;
   ...
   //ATHULYAS COMMENTS: INITIALIZE THE MUTEX FOR WAITERS 
